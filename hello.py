...
app.config['SQLALCHEMY_BINDS'] = {
    'hobbies': 'sqlite:///hobbies.db', 'details': 'sqlite:///details.db'}
...
# create another form Hobby class
...
# create add_details route and function
...
def add_details():
    ...
    form = DetailsForm()
    if form.validate_on_submit():
        detail = Details.query.filter_by(name=form.name.data).first()
        if detail is None:
            detail = Details(name=form.name.data,
        ...
        form.email.data = ''
        form.address.data = ''
        flash('Detail added successfully')
    detail_lists = Details.query.order_by(Details.date_added)
    return render_template("add_details.html", form=form, name=name, email=email, address=address, detail_lists=detail_lists)
